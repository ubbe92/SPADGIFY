...
function MusicStreamer() {
  ...
  return (
      <div className="music-streaming-client">
        <div hidden={isNotStarted}>
          <div><strong> Requested song:</strong> {value}</div>
        </div>
        ...
        <div>
          ...
          ></SocketInput>
          <div style={{marginBottom: "1rem"}} hidden={!isNotStarted}>Socket server: {ipPortSocket}</div>
          <SocketInput
            getIpPort={getIpPortRest}
            hidden={!isNotStarted}
            defaultValue={ipPortRest}
          ></SocketInput>
          <div style={{marginBottom: "1rem"}} hidden={!isNotStarted}>Rest server: {ipPortRest}</div>
        </div>

        <Button variant="outline-dark" onClick={initPlayer} hidden={!isNotStarted} >
          Init player
        </Button>

        <Table getMedia={getMedia} data={data}></Table>
        <div class="media-button-panel">
            <button class="fa fa-play" onClick={startAudio} disabled={isPlaying} hidden={isNotStarted}></button>
            <button class="fa fa-stop" onClick={stopAudio} disabled={!isPlaying} hidden={isNotStarted}></button>
